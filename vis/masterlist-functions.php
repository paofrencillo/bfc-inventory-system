...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Delete Product selected by the User
    else if (isset($_POST['action']) && $_POST['action'] === 'delete_items') {
        $prod_ids = explode(',', $_POST['prod_ids']);
        // $temp = explode("''",  $prod_ids);
        $table = "inventory";
        $table2 = "product_masterlist";
        $data = ($prod_ids);

        // Create a new PDO connection to your MySQL database
        $pdo = new PDO('mysql:host=localhost;dbname=bfc_Inventory', 'root', '');

        // Prepare a SQL query with placeholders for the array values
        $stmt = $pdo->prepare("DELETE FROM $table WHERE id IN (" . implode(",", array_fill(0, count($prod_ids), "?")) . ")");
        // Prepare a SQL query with placeholders for the array values
        $stmt2 = $pdo->prepare("DELETE FROM $table2 WHERE id IN (" . implode(",", array_fill(0, count($prod_ids), "?")) . ")");

        // Bind the array values to the prepared statement
        $stmt->execute($prod_ids);
        $stmt2->execute($prod_ids);
        echo json_encode('success');
    }
